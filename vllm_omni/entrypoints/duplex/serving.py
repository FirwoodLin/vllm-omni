...
class OmniDuplexSessionHandler(
    ...
    async def _handle_playback_ack(self, session: DuplexSession, event: dict[str, object], send_json) -> None:
        ...
        try:
            session.playback_for_response(response_id)
        except KeyError:
            await send_json(
                {
                    "type": "error",
                    "session_id": session.session_id,
                    "epoch": session.epoch,
                    "code": "unknown_playback_response",
                    "error": f"Unknown playback response_id: {response_id}",
                }
            )
            return
            ...
            await send_json(
                {
                    ...
                    "session_id": session.session_id,
                    "epoch": session.epoch,
                    "code": "playback_item_conflict",
                    "error": "playback.ack item_id conflicts with response_id",
                }
            )
            return
